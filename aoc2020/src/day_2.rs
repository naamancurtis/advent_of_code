...
pub trait IsValidPassword {
    fn new(input: (&str, &str)) -> Self;
    fn is_valid(&self) -> bool;
}

mod puzzle_1 {
    use super::IsValidPassword;
    ...
    impl IsValidPassword for PasswordDetails {
        fn new(input: (&str, &str)) -> Self {
            ...
            let max: i8 = min_max.next().unwrap().parse().unwrap();
            let letter = requirements.next().unwrap().chars().next().unwrap();

            Self {
                min,
                max,
                letter,
                password: input.1.to_string(),
            }
        }
...
pub fn count_valid_passwords<T: IsValidPassword>(input: &[(&str, &str)]) -> u32 {
    input
        .iter()
        .filter_map(|i| {
            let details = T::new(*i);
...
mod tests {
    ...
    fn puzzle_1_test() {
        ...
        assert_eq!(
            count_valid_passwords::<puzzle_1::PasswordDetails>(&input),
            2
        );
    }
    ...
    fn puzzle_2_test() {
        ...
        assert_eq!(
            count_valid_passwords::<puzzle_2::PasswordDetails>(&input),
            1
        );
    }
